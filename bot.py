...
class DataStorage:
    ...
    def ensure_files_exist(self):
        """Создает пустые файлы если их нет"""
        files = [
            self.products_file,
    ...
    def load_users(self):
        """Загрузить список пользователей для рассылки"""
        try:
    ...
    def add_user(self, user_id: int, username: str = ""):
        """Добавить пользователя для рассылки"""
        try:
    ...
    def bulk_update_city_orders(self, order_mapping: Dict[int, int]):
        """Обновить порядок нескольких городов"""
        try:
    ...
    def get_city_products(self, city_id: int) -> List[Product]:
        """Получить товары, добавленные напрямую в город"""
...
# ======================= ДОБАВЛЕНИЕ ТОВАРА В ГОРОД БЕЗ КАТЕГОРИИ =======================
...
async def lifespan(app):
    ...
    # Регистрируем все хэндлеры (они уже привязаны к глобальному dp)
    # Так как все декораторы используют глобальный dp, они уже зарегистрированы
...
routes = [
    ...
    Route("/", health_check, methods=["GET"]),  # Корневой путь тоже отдаем OK
]
